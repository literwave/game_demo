...
local exception = lpeg.Cmt(lpeg.Carg(1), function(text, pos, parser_state)
...
local function namedpat(n, pat)
	return Ct(Cg(Cc(n), "type") * Cg(pat))
end
...
local typedef = P {
	...
	FIELD = namedpat(
		"field",
		(name * blanks * (
			namedpat("ref", typename)
			+ namedpat("list", P"[" * typename * P"]")
			+ namedpat("map", P"<" * typename * blank0 * "," * blank0 * typename * P">")
			+ namedpat("struct", P"{" * multipat(V"FIELD") * P"}")
		))
	),
	STRUCT = namedpat("struct", blank0 * name * blank0 * P"{" * multipat(V"FIELD") * P"}"),
	LIST = namedpat("list", blank0 * name * blank0 * P"[" * typename * P"]"),
	MAP = namedpat("map", blank0 * name * blank0 * P"<" * typename * blank0 * "," * blank0 * typename * P">"),
	...
	for line in io.lines(path) do
		...
		if not include then
			...
			for _line in io.lines(include) do
				...
				local tidx = #text + 1
				text[tidx] = _line
				line_infos[tidx] = {line = _idx, file = include}
			end
		end
	end
...
function M.parse(filename, dir)
	return parse(schema, filename, dir)
end
